...
- With `goal: civilization_wins`, `goal_civilizations` is the complete civilization pool and `civilization_pool` is ignored. The client creates and sends one numbered check for every required win with every selected civilization. For example, English, French, and Malians with five wins per civilization creates exactly 15 checks: wins 1-5 for each civ. `civ_sanity`, `win_check_interval`, and `win_check_count` are ignored for this goal.
- With `goal: total_wins`, `goal: solo_rank`, or `goal: team_rank`, `civilization_pool` controls the starting civilization and unlock items, while `goal_civilizations` is ignored. Civilization sanity and global win-interval checks retain their configured behavior.

`starting_civs` controls how many civilizations from the goal's active list begin unlocked and defaults to 1. It cannot exceed the size of that list. A specifically selected `starting_civilization` must be in the active list and is guaranteed to be among the starting unlocks; `random` resolves the first unlock from that list. Any additional starting civilizations are selected from the remaining active civilizations using the seed's deterministic randomization.
...
The **Tracker** tab updates automatically as civilization unlock items arrive and as AOE4World wins are credited. Its first section lists only unlocked civilizations that still need wins for civilization sanity or the civilization-wins goal. For a civilization-wins seed, this progress matches the numbered checks for each goal civilization. The available section shows every civilization you may currently play for credit, including completed per-civilization progress, and the locked section previews the rest of the active pool. The packaged flag artwork comes from AOE4World Explorer, so no separate image download is needed while playing.
